...
def test():
    ...
    with tf.Session() as sess:
        ...
        yP = sess.run([mainN.probs], feed_dict={mainN.data: test_dataset})[0]
        # yP, LO = sess.run([mainN.probs, mainN.label_oh],
        #     feed_dict={mainN.data: test_dataset, mainN.labels: test_labels})
        print("shape: ", yP.shape, labels.shape)
        print('test accuracy: %.1f%%' % accuracy(np.asarray(yP), np.asarray(labels)))
